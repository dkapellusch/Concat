...
internal class CommandLineArgs
{
    ...
    [Option('l', "compression-level", Required = false, HelpText = "Set the compression level (0-4)", Default = CompressionLevel.Medium)]
    public CompressionLevel CompressionLevel { get; set; } = CompressionLevel.Medium;

    [Option('p', "compress", Required = false, HelpText = "Compress the output for LLM consumption", Default = false)]
    public bool CompressOutput { get; set; }

    [Option('h', "ignore-hidden", Required = false, HelpText = "Ignore files and folders that begin with a '.'", Default = true)]
